...
- (void)viewDidLoad {
    ...
    [self viewSetUp];//UI
     [[HGBTabBarBadgeTool shareInstance] setTabBarBadgeColor:[UIColor redColor] inController:self];
}
#pragma mark 导航栏
//导航栏
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.section==0){
        if(indexPath.row==0){
            [[HGBTabBarBadgeTool shareInstance] setTabBarPointColor:[UIColor blueColor] inController:self];
            NSLog(@"设置成功");
        }else if (indexPath.row==1){
            [[HGBTabBarBadgeTool shareInstance] addPointInController:self];
            NSLog(@"设置成功");
        }else if (indexPath.row==2){
            [[HGBTabBarBadgeTool shareInstance] hideTabBarPointInController:self];
            NSLog(@"设置成功");
        }
    }
}
