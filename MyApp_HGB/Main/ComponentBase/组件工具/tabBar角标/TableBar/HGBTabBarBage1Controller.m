...
- (void)viewDidLoad {
    ...
    [self viewSetUp];//UI
    [[HGBTabBarBadgeTool shareInstance] setTabBarBadgeColor:[UIColor yellowColor] inController:self];
...
-(void)viewSetUp{
    ...
    self.dataDic=@{@"tabBar角标:":@[@"角标设置为1",@"角标+1",@"角标-1",@"角标隐藏",@"角标加5",@"获取角标"]};
    self.keys=@[@"tabBar角标:"];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.section==0){
        if(indexPath.row==0){
            [[HGBTabBarBadgeTool shareInstance] setTabBarBadge:@"1" inController:self];
            NSLog(@"设置成功");
        }else if (indexPath.row==1){
            [[HGBTabBarBadgeTool shareInstance] addTabBarBadgeInController:self];
            NSLog(@"设置成功");
        }else if (indexPath.row==2){
            [[HGBTabBarBadgeTool shareInstance] reduceTabBarBadgeInController:self];
            NSLog(@"设置成功");
        }else if (indexPath.row==3){
            [[HGBTabBarBadgeTool shareInstance] hideTabBarBadgeInController:self];
            NSLog(@"设置成功");
        }else if (indexPath.row==3){
            [[HGBTabBarBadgeTool shareInstance] hideTabBarBadgeInController:self];
            NSLog(@"设置成功");
        }else if (indexPath.row==4){
            [[HGBTabBarBadgeTool shareInstance] addTabBarBadgeWithNumber:5 InController:self];
        }else if (indexPath.row==5){
            NSLog(@"角标:%@",[[HGBTabBarBadgeTool shareInstance] getTabBarBadgeInController:self]);
        }
    }
}
