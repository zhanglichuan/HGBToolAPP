...
@interface HGBJumpViewController ()
/**
 数据源
 */
...
@implementation HGBJumpViewController
...
-(void)viewSetUp{
    ...
    self.dataDic=@{@"控制器跳转:":@[@"跳转Pop-Push",@"跳转Present-Dismiss"]};
    self.keys=@[@"控制器跳转:"];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.section==0){
        if(indexPath.row==0){
            [self presentControllerWithNav:[[HGBPP1ViewController alloc]init]];
        }else if (indexPath.row==1){
            [self presentControllerWithNav:[[HGBPD1ViewController alloc]init]];
