...
@interface HGBNavTestViewController ()
/**
 数据源
 */
...
@implementation HGBNavTestViewController
...
-(void)viewSetUp{
    ...
    self.dataDic=@{@"导航栏样式:":@[@"修改当前导航栏颜色",@"修改当所有导航栏颜色",@"导航栏隐藏",@"导航栏显示"],@"导航栏:":@[@"导航栏1",@"导航栏2"]};
    self.keys=@[@"导航栏样式:",@"导航栏:"];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section==0){
        if(indexPath.row==0){
            [HGBNavigationBarTool changeNavgationBarColor:[UIColor redColor]];
        }else if (indexPath.row==1){
            [HGBNavigationBarTool changeAllNavgationBarColor:[UIColor redColor]];
        }else if (indexPath.row==2){
            [HGBNavigationBarTool setNavgationBarIsHidden:YES];
        }else if (indexPath.row==3){
            [HGBNavigationBarTool setNavgationBarIsHidden:NO];
        ...
        if(indexPath.row==0){
            ...
            [self presentControllerWithNav:[[HNav1ViewController alloc]init]];
        }else if (indexPath.row==1){
            [self presentControllerWithNav:[[HNav2ViewController alloc]init]];
