...
-(void)viewSetUp{
    ...
    self.dataDic=@{@"application角标:":@[@"角标设置为1",@"角标+1",@"角标-1",@"角标隐藏",@"角标获取",@"角标添加"]};
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.section==0){
        if(indexPath.row==0){
            [HGBApplicationBadgeTool setApplicationBadge:1];
            NSLog(@"设置成功");
        }else if (indexPath.row==1){
            [HGBApplicationBadgeTool addApplicationBadge];
            NSLog(@"设置成功");
        }else if (indexPath.row==2){
            [HGBApplicationBadgeTool reduceApplicationBadge];
            NSLog(@"设置成功");
        }else if (indexPath.row==3){
            [HGBApplicationBadgeTool hideApplicationBadge];
            NSLog(@"设置成功");
        }else if (indexPath.row==4){
            NSLog(@"角标:%ld",[HGBApplicationBadgeTool getApplicationBadge]);
        }else if (indexPath.row==5){
            [HGBApplicationBadgeTool addApplicationBadgeWithNumber:3];
            NSLog(@"设置成功");
        }
    }
}
