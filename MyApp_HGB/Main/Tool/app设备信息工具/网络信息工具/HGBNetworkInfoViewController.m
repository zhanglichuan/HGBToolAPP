//
//  HGBNetworkInfoViewController.m
//  MyApp_HGB
//
...
@interface HGBNetworkInfoViewController ()
/**
 数据源
 */
...
@implementation HGBNetworkInfoViewController
...
-(void)viewSetUp{
    ...
    self.dataDictionary=@{@"网络信息工具":@[@"网络类型",@"网络强度",@"流量"]};
    self.keys=@[@"网络信息工具"];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.section==0){
        ...
        if (indexPath.row==0){
            HGBNetworkType type=[HGBNetworkInfoTool getNetworkType];
            if(type==HGBNetworkTypeNone){
                NSLog(@"无网络");
            }else if (type==HGBNetworkTypeWIFI){
                NSLog(@"WIFI");
            }else if (type==HGBNetworkType4G){
                NSLog(@"4G");
            }else if (type==HGBNetworkType3G){
                NSLog(@"3G");
            }else if (type==HGBNetworkType2G){
                NSLog(@"2G");
            }
        }else if (indexPath.row==1){
            NSLog(@"网络强度:%d",[HGBNetworkInfoTool getSignalStrength]);

        }else if (indexPath.row==2){
            NSLog(@"%@",[HGBNetworkInfoTool getFlowIOBytesWithUnitType:YES]);
