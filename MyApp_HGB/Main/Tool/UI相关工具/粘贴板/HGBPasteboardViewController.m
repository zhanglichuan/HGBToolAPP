...
@interface HGBPasteboardViewController ()
/**
 数据源
 */
...
@implementation HGBPasteboardViewController
...
-(void)viewSetUp{
    ...
    self.dataDictionary=@{@"粘贴板":@[@"复制",@"粘贴板"]};
    self.keys=@[@"粘贴板"];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.section==0){
        ...
        if (indexPath.row==0){
            [HGBPasteboardTool pasteSource:@"123"];
        }else if (indexPath.row==1){
            NSLog(@"%@",[HGBPasteboardTool getPasteboardSource]);
        }
