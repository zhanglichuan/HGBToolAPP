//
//  HGBAccelerometerViewController.m
//  MyApp_HGB
//
//  Created by huangguangbao on 2018/1/14.
...
@interface HGBAccelerometerViewController ()
/**
 数据源
 */
...
@implementation HGBAccelerometerViewController
...
-(void)viewSetUp{
    ...
    self.dataDictionary=@{@"距离传感器":@[@"打开距离传感器",@"关闭",@"滚动的小球"]};
    self.keys=@[@"距离传感器"];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.section==0){
        ...
        if (indexPath.row==0){
            [[HGBAccelerometerTool shareInstance] setTimeInterval:1];
            [[HGBAccelerometerTool shareInstance] startAccelerometerWithReslut:^(BOOL status, NSDictionary *returnMessage) {
                 NSLog(@"%@",returnMessage);
            }];
        }else if(indexPath.row==1){
            [[HGBAccelerometerTool shareInstance] stopAccelerometerWithReslut:^(BOOL status, NSDictionary *returnMessage) {
            }];
        }else if(indexPath.row==2){
            [self presentController:[[HGBRollBalViewController alloc]init]];
        }
