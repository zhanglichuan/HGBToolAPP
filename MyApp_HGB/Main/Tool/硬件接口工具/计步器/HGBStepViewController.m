...
@interface HGBStepViewController ()
/**
 数据源
 */
...
@implementation HGBStepViewController
...
-(void)viewSetUp{
    ...
    self.dataDictionary=@{@"计步器":@[@"计步器",@"关闭",@"获取运动状态",@"获取健康数据"]};
    self.keys=@[@"计步器"];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.section==0){
        ...
        if (indexPath.row==0){
            [[HGBStepTool shareInstance] setTimeInterval:1];
            [[HGBStepTool shareInstance] startMonitortStepWithReslut:^(BOOL status, NSDictionary *returnMessage) {
                 NSLog(@"%@",returnMessage);
            }];
        }else if(indexPath.row==1){
            [[HGBStepTool shareInstance] stopMonitorStepWithReslut:^(BOOL status, NSDictionary *returnMessage) {
                NSLog(@"%@",returnMessage);
            }];
            [[HGBStepTool shareInstance] stopMonitorpActivityStatusWithReslut:^(BOOL status, NSDictionary *returnMessage) {
                NSLog(@"%@",returnMessage);
            }];
        }else if(indexPath.row==2){
            [[HGBStepTool shareInstance] startMonitorActivityStatuspWithReslut:^(BOOL status, NSDictionary *returnMessage) {
                NSLog(@"%@",returnMessage);
            }];

        }else if(indexPath.row==3){
            [[HGBStepTool shareInstance] queryStepDataFromDate:[NSDate date] andWithReslut:^(BOOL status, NSDictionary *returnMessage) {
                NSLog(@"%@",returnMessage);
            }];
        }
