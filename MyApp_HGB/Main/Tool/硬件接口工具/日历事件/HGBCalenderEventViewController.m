//
//  HGBCalenderEventViewController.m
//  MyApp_HGB
//
...
@interface HGBCalenderEventViewController ()
/**
 数据源
 */
...
@implementation HGBCalenderEventViewController
...
-(void)viewSetUp{
    ...
    self.dataDictionary=@{@"日历事件":@[@"添加",@"获取"]};
    self.keys=@[@"日历事件"];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.section==0){
        ...
        if (indexPath.row==0){
            [HGBCalenderEventTool saveEventCalendarTitle:@"测试" location:@"测试" startDate:[NSDate date] endDate:[NSDate date] allDay:YES alarmArray:nil andWithReslut:^(BOOL status, NSDictionary *returnMessage) {
                NSLog(@"%@",returnMessage);
            }];
        }else if(indexPath.row==1){
            NSArray *array=[HGBCalenderEventTool fetchEventsWithStartDate:[NSDate date] endDate:[NSDate date]];
            NSLog(@"%@",array);
