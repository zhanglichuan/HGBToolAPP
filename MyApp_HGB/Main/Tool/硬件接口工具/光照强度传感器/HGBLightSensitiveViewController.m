//
//  HGBLightSensitiveViewController.m
//  MyApp_HGB
//
...
@interface HGBLightSensitiveViewController ()<HGBLightSensitiveToolDelegate>
/**
 数据源
 */
...
@implementation HGBLightSensitiveViewController
...
-(void)viewSetUp{
    ...
    self.dataDictionary=@{@"光线强度":@[@"光线强度",@"关闭"]};
    self.keys=@[@"光线强度"];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.section==0){
        ...
        if (indexPath.row==0){
            [[HGBLightSensitiveTool shareInstance] setDelegate:self];
            [[HGBLightSensitiveTool shareInstance] setTimeInterval:1];
            [[HGBLightSensitiveTool shareInstance] startMonitLightSensitiveWithRepeat:YES];
        }else if(indexPath.row==1){
            [[HGBLightSensitiveTool shareInstance] stopMonitLightSensitive];
        }
