//
//  HGBReminderEventViewController.m
//  MyApp_HGB
//
...
@interface HGBReminderEventViewController ()
/**
 数据源
 */
...
@implementation HGBReminderEventViewController
#pragma mark life
- (void)viewDidLoad {
    [super viewDidLoad];
...
-(void)viewSetUp{
    ...
    self.dataDictionary=@{@"事件提醒":@[@"添加",@"获取"]};
    self.keys=@[@"事件提醒"];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.section==0){
        ...
        if (indexPath.row==0){
            [HGBReminderEventTool saveEventIntoReminderWithTitle:@"测试" notes:@"测试" startDate:[NSDate date] endDate:[NSDate date] alarm:nil priority:1 completed:YES reslutBlock:^(BOOL status, NSDictionary *returnMessage) {
                NSLog(@"%@",returnMessage);
            }];
        }else if(indexPath.row==1){
            [HGBReminderEventTool fetchAllRemindersWithReslut:^(BOOL status, NSArray *eventArr) {
                NSLog(@"%@",eventArr);
            }];
