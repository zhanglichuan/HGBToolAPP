...
@interface HGBDeviceSetViewController ()
/**
 数据源
 */
...
@implementation HGBDeviceSetViewController
#pragma mark life
- (void)viewDidLoad {
    [super viewDidLoad];
...
-(void)viewSetUp{
    ...
    self.dataDictionary=@{@"设备小功能":@[@"获取屏幕亮度",@"设置屏幕亮度",@"震动",@"播发系统声音"]};
    self.keys=@[@"设备小功能"];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.section==0){
        ...
        if (indexPath.row==0){
            NSLog(@"屏幕亮度:%f",[HGBDeviceTool getScreenBrightness]);
        }else if(indexPath.row==1){
            [HGBDeviceTool setScreenBrightness:0.7];
        }else if(indexPath.row==2){
            [HGBDeviceTool vibrate];
        }else if(indexPath.row==3){
            [HGBDeviceTool playSystemSoundWithSoundId:1007];
        }
