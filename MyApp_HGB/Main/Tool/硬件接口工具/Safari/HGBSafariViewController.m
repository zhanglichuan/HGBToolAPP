...
@interface HGBSafariViewController ()
/**
 数据源
 */
...
@implementation HGBSafariViewController
#pragma mark life
- (void)viewDidLoad {
    [super viewDidLoad];
...
-(void)viewSetUp{
    ...
    self.dataDictionary=@{@"Safari":@[@"打开Safari浏览器",@"添加Safari书签"]};
    self.keys=@[@"Safari"];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.section==0){
        [HGBSafariTool openURL:@"https://www.baidu.com"];

        if (indexPath.row==0){
            [HGBSafariTool addBookmarkWithTitle:@"测试" andWithPrompt:@"测试内容" andWithURL:@"https://www.baidu.com"];
