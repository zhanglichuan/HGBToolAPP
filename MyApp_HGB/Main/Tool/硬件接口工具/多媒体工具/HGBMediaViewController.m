...
-(void)viewSetUp{
    ...
    self.dataDictionary=@{@"多媒体工具:":@[@"打开手电筒",@"关闭手电筒",@"相册",@"拍照",@"录像",@"开始录音",@"暂停录音",@"结束录音",@"取消录音",@"打开图片",@"播放视频",@"播放音频",@"震动",@"播放wav"]};
    self.keys=@[@"多媒体工具:"];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.section==0){
        ...
        if (indexPath.row==0){
            ...
            [media lookImageAtSource:self.imagePath inParent:self];
        }else if (indexPath.row==10){
            [media lookVideoAtSource:self.videoPath inParent:self];
        }else if (indexPath.row==11){
            [media lookAudioAtSource:self.audioPath inParent:self];
        }else if (indexPath.row==12){
            [media vibrate];
        }else if (indexPath.row==13){
            [media playWavWithSource:@"project://test.aac"];
        }
