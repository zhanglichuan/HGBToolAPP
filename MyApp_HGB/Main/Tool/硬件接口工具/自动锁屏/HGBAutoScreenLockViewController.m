//
//  HGBAutoScreenLockViewController.m
//  MyApp_HGB
//
...
@interface HGBAutoScreenLockViewController ()
/**
 数据源
 */
...
@implementation HGBAutoScreenLockViewController
...
-(void)viewSetUp{
    ...
    self.dataDictionary=@{@"自动锁屏":@[@"开启",@"关闭"]};
    self.keys=@[@"自动锁屏"];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.section==0){
        ...
        if (indexPath.row==0){
            [HGBScreenLockTool openAutoScreenLock];
        }else if(indexPath.row==1){
            [HGBScreenLockTool closeAutoScreenLock];
