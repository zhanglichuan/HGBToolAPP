...
-(void)viewSetUp{
    ...
    self.dataDictionary=@{@"WIFI":@[@"WIFI的IP地址",@"WIFI的名称",@"WIFI的网络名称",@"打开WIFI设置",@"ping-baidu"]};
    self.keys=@[@"WIFI"];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.section==0){
        ...
        if (indexPath.row==0){
            NSLog(@"%@",[HGBWIFITool getLocalWiFiIPAddress]);
        }else if (indexPath.row==1){
             NSLog(@"%@",[HGBWIFITool getWIFIName]);
        }else if (indexPath.row==2){
           NSLog(@"%@",[HGBWIFITool getCurrentWifiSSID]);
        }else if (indexPath.row==3){
            NSLog(@"%@",[HGBWIFITool getCurrentWifiSSID]);
        }else if (indexPath.row==4){
            [HGBWIFITool pingUrl:@"www.baidu.com" withReslutBlock:^(BOOL status, NSDictionary *returnMessage) {
