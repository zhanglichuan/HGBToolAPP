//
//  HGBSystemShareViewController.m
//  MyApp_HGB
//
...
@interface HGBSystemShareViewController ()
/**
 数据源
 */
...
@implementation HGBSystemShareViewController
#pragma mark life
- (void)viewDidLoad {
    [super viewDidLoad];
...
-(void)viewSetUp{
    ...
    self.dataDictionary=@{@"系统分享":@[@"分享选择窗口",@"普通分享",@"系统分享"]};
    self.keys=@[@"系统分享"];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.section==0){
        ...
        if (indexPath.row==0){
            [HGBShareTool shareSource:@[@"title",[NSURL URLWithString:@"https://www.baidu.com"],[UIImage imageNamed:@"icon"],[UIImage imageNamed:@"icon"]] inParent:self andWithResult:^(BOOL status, NSDictionary *returnMessage) {
                NSLog(@"%@",returnMessage);
            }];
        }else if(indexPath.row==1){
            [HGBShareTool shareWithTitle:@"title" andWithURL:@"https://www.baidu.com" andWithImage:[UIImage imageNamed:@"icon"] inParent:self andWithResult:^(BOOL status, NSDictionary *returnMessage) {
                NSLog(@"%@",returnMessage);
            }];
        }else if(indexPath.row==2){
            [HGBShareTool slshareWithShareType:(HGBSLShareTypeSinaWeibo) andWithTitle:@"测试" andWithImages:@[[UIImage imageNamed:@"icon"]] andWithURLs:@[@"https://www.baidu.com"] inParent:self andWithResult:^(BOOL status, NSDictionary *returnMessage) {
                NSLog(@"%@",returnMessage);
            }];
        }
