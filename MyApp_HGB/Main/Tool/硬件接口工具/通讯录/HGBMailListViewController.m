...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.section==0){
        ...
        if (indexPath.row==0){
            [[HGBMailListTool shareInstance] openMailListBookInParent:self andWithDelegate:self];

        }else if (indexPath.row==1){
            [[HGBMailListTool shareInstance] getMailListWithDelegate:self];
        }else if (indexPath.row==2){
            [[HGBMailListTool shareInstance] creatItemWithName:@"" phone:@"18812345678" andWithDelegate:self];
        }
...
-(void)mailList:(HGBMailListTool *)mailList didSucessedWithMail:(HGBMailModel *)mail{
    NSLog(@"%@",mail.phoneNumber);
    NSLog(@"%@",mail.phoneNumbers);
}
