...
@implementation HGBBluetoothTool

+(HGBBluetoothTool*)shareInstance
{
    static dispatch_once_t pred = 0;
    __strong static HGBBluetoothTool *_sharedObject = nil;
    dispatch_once(&pred, ^{
...
-(void)startScan
{
    ...
    {
        if(self.delegate&&[self.delegate respondsToSelector:@selector(bluetoothTool:didFailedWithError:)]){
            [self.delegate bluetoothTool:self didFailedWithError:@{ReslutCode:@(0),ReslutMessage:@"请到系统设置中打开蓝牙"}];
        }
    }
...
-(BOOL)sendCommand:(HGB_CommandType)command
{
    ...
    if([self isReady])
    {
        ...
        *(data+2) = 0x01;
        ...
        switch (command) {
            case HGB_Command_Start:
            {

            }
                break;
            case HGB_Command_Stop:
            {
                command = 0x00;
            }
                break;
            case HGB_Command_Mode1:
                break;
            case HGB_Command_Mode2:
                break;
            case HGB_Command_Mode3:
                break;
            case HGB_Command_Mode4:
            case HGB_Command_Mode5:
            case HGB_Command_Mode6:
            case HGB_Command_Mode7:
            case HGB_Command_Mode8:
            case HGB_Command_Mode9:
            {
                //                datastr = [NSString stringWithFormat:@"%d",command];
            }
                break;
            case HGB_Command_Shutdown:
            {
                command = 0x5a;
            }
            default:
