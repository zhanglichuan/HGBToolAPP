//
//  HGBScreenBrightnessViewController.m
//  MyApp_HGB
//
...
@interface HGBScreenBrightnessViewController ()
/**
 数据源
 */
...
@implementation HGBScreenBrightnessViewController
#pragma mark life
- (void)viewDidLoad {
    [super viewDidLoad];
...
-(void)viewSetUp{
    ...
    self.dataDictionary=@{@"屏幕亮度":@[@"获取屏幕亮度",@"设置屏幕亮度"]};
    self.keys=@[@"屏幕亮度"];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.section==0){
        ...
        if (indexPath.row==0){
            NSLog(@"屏幕亮度:%f",[HGBScreenBrightnessTool getScreenBrightness]);
        }else if(indexPath.row==1){
            [HGBScreenBrightnessTool setScreenBrightness:0.7];
        }
