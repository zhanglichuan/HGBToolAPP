...
@interface HGBDistanceViewController ()<HGBDistanceSensorToolDelegate>
/**
 数据源
 */
...
@implementation HGBDistanceViewController
...
-(void)viewSetUp{
    ...
    self.dataDictionary=@{@"距离传感器":@[@"打开距离传感器",@"关闭"]};
    self.keys=@[@"距离传感器"];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.section==0){
        ...
        if (indexPath.row==0){
            [[HGBDistanceSensorTool shareInstance] setDelegate:self];

            [[HGBDistanceSensorTool shareInstance] startMonitorDistance];
        }else if(indexPath.row==1){
            [[HGBDistanceSensorTool shareInstance] stopMonitorDistance];
        }
