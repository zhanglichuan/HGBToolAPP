...
@interface HGBBackgroundViewController ()
/**
 数据源
 */
...
@implementation HGBBackgroundViewController
...
-(void)viewSetUp{
    ...
    self.dataDictionary=@{@"后台执行":@[@"点击后台执行开始,请切换后台"]};
    self.keys=@[@"后台执行"];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.section==0){
        ...
        if (indexPath.row==0){
            [[HGBBackgroundTaskTool shareInstance] startBackgroundTask];
