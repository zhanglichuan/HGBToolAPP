...
@interface HGBURLToolViewController ()
/**
 数据源
 */
...
@implementation HGBURLToolViewController
...
-(void)viewSetUp{
    ...
    self.dataDictionary=@{@"URL工具":@[@"获取url参数",@"获取home路径"]};
    self.keys=@[@"URL工具"];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.section==0){
        ...
        if (indexPath.row==0){
            NSLog(@"%@",[HGBURLTool getParamsFromURL:@"http://www.baidu.com?name=huang&age=27"]);
        }else if (indexPath.row==1){
            NSLog(@"%@",[HGBURLTool urlAnalysis:@"home://"]);
