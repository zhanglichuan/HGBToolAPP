...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.section==0){
        if (indexPath.row==0){
            ...
            NSLog(@"%@",[HGBXMLReader XMLObjectWithXmlFile:@"project://test.xml"]);
        }else if (indexPath.row==1){
            NSDictionary *dic=@{@"items":@{@"name":@"huang",@"age":@"27"}};
            NSLog(@"%@",[HGBXMLWriter XMLStringFromObject:dic]);
            NSLog(@"%d",[HGBXMLWriter XMLFileFromObject:dic withHeader:YES toDestinationFile:@"document://1.xml" Error:nil ]);
        }
