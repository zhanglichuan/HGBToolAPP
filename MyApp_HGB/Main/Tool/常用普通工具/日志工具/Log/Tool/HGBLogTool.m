...
- (void)redirectLogToDocumentFolder
{
    ...
    NSString *lastLogFilePath=[HGBLogTool urlAnalysisToPath:logFilePath];
    NSString *directoryPath=[lastLogFilePath stringByDeletingLastPathComponent];
    if(![HGBLogTool isExitAtFilePath:directoryPath]){
        [HGBLogTool createDirectoryPath:directoryPath];
    }

    // 先删除已经存在的文件
    NSFileManager *defaultManager = [NSFileManager defaultManager];
    [defaultManager removeItemAtPath:lastLogFilePath error:nil];
    // 将NSLog的输出重定向到文件，因为C语言的printf打印是往stdout打印的，这里也把它重定向到文件
    freopen([lastLogFilePath cStringUsingEncoding:NSASCIIStringEncoding],"a+", stdout);
    freopen([lastLogFilePath cStringUsingEncoding:NSASCIIStringEncoding],"a+", stderr);
}
#pragma mark 获取日志列表
/**
 获取日志路径列表文件路径
...
+(BOOL)moveFilePath:(NSString *)srcPath ToPath:(NSString *)filePath{
    if(![HGBLogTool isExitAtFilePath:srcPath]){
        return NO;
    }
...
+(BOOL)urlExistCheck:(NSString *)url{
    ...
    if(![HGBLogTool isURL:url]){
        return NO;
    }
    url=[HGBLogTool urlAnalysis:url];
...
+(NSString *)urlAnalysisToPath:(NSString *)url{
    ...
    NSString *urlstr=[HGBLogTool urlAnalysis:url];
