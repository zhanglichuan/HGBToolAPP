//
//  HGBAppManagerController.m
//  MyApp_HGB
//
//  Created by huangguangbao on 2018/5/21.
//  Copyright © 2018年 agree.com.cn. All rights reserved.
//

#import "HGBAppManagerController.h"
#import "HGBCommonSelectCell.h"
#define Identify_Cell @"cell"
#import "HGBAppManager.h"

@interface HGBAppManagerController ()
/**
 数据源
 */
...
@implementation HGBAppManagerController
...
-(void)viewSetUp{
    ...
    self.dataDictionary=@{@"App管理":@[@"获取全部app列表",@"获取系统app列表",@"获取安装app列表",@"app是否已安装",@"使用bundleid打开app"]};
    self.keys=@[@"App管理"];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.section==0){
        ...
        if (indexPath.row==0){
            NSArray *appList=[[HGBAppManager shareInstance]getAllAppList];
            ...
            NSArray *appList=[[HGBAppManager shareInstance]getAllSystemAppList];
            for(NSDictionary *dic in appList){
                NSLog(@"%@",dic);
            }
        }else if (indexPath.row==2){
            NSArray *appList=[[HGBAppManager shareInstance]getAllInstalledAppList];
            for(NSDictionary *dic in appList){
                NSLog(@"%@",dic);
            }
        }else if (indexPath.row==3){
            NSLog(@"%d",[[HGBAppManager shareInstance]isInstalledWithAppBundleId:@"com.agree.NativeWork"]);
        }else if (indexPath.row==4){
            [[HGBAppManager shareInstance]openAppWithAppBundleId:@"com.agree.NativeWork"];
        }
