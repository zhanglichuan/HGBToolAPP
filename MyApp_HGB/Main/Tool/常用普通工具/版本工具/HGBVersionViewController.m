...
@interface HGBVersionViewController ()
/**
 数据源
 */
...
@implementation HGBVersionViewController
...
-(void)viewSetUp{
    ...
    self.dataDictionary=@{@"版本工具:":@[@"app下载",@"打开appstore",@"获取appstore的应用信息",@"应用评价"]};
    self.keys=@[@"版本工具:"];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.section==0){
        if (indexPath.row==0){
            [HGBVersionTool downLoadAppWithManifestPlistUrl:@"https://plist.hnchewutong.cn/8c128ce4-2e29-44e3-8ea5-17706adc706d_1_00_009.plist"];
        }else if(indexPath.row==1){
            [[HGBVersionTool shareInstance]  openAppInAppStoreWithAppID:@"1042178917" andWithReslut:^(BOOL status, NSDictionary *message) {
                NSLog(@"%@",message);
            }];
        }else if(indexPath.row==2){
            [HGBVersionTool  getAppInfoFromAppStoreWithAppID:@"1042178917" andWithReslut:^(BOOL status, NSDictionary *message) {
                 NSLog(@"%@",message);
            }];
        }else if(indexPath.row==3){
            [HGBVersionTool  openAppVerbInAppStoreWithAppID:@"1042178917"];
        }
