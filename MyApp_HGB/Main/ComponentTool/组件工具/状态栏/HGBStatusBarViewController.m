...
@interface HGBStatusBarViewController ()
/**
 数据源
 */
...
@implementation HGBStatusBarViewController
...
-(void)viewSetUp{
    ...
    self.dataDic=@{@"状态栏:":@[@"状态栏隐藏",@"状态栏显示",@"修改状态栏样式透明",@"状态栏样式普通"]};
    self.keys=@[@"状态栏:"];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.section==0){
        if (indexPath.row==0){
            [HGBStatusBarTool setStatusBarIsHidden:YES];
        }else if (indexPath.row==1){
            [HGBStatusBarTool setStatusBarIsHidden:NO];

        }else if (indexPath.row==2){
            [HGBStatusBarTool setStatusBarStyle:(UIStatusBarStyleLightContent)];
        }else if (indexPath.row==3){
            [HGBStatusBarTool setStatusBarStyle:(UIStatusBarStyleDefault)];
        }
    }
}
